...
//
// pgvector is required for PostgreSQL rule embeddings. If the extension cannot
// be created, return an error so DB.Migrate fails closed instead of starting
// with an incomplete rule_embeddings schema.
